...
if(isset($_SESSION['patient_rec_edit']))
{
    if($_SESSION['patient_rec_edit']==1)
    {
        ...
        // require('../../includes/fpdf184/pdf_mc_table.php');
        ...
        $data=$generate_report_obj->report_month($month,$year);
        if($data!=0)
        {
            ...
            $pdf->Cell(190,5,'',0,1,);
            $pdf->Cell(190,0,$type.' Operations Summary Report',0,1,'C');
            $pdf->Cell(190,5,'',0,1);
            for($i=0;$i<$data_size;$i++)
            {
                if($data[$i]['id']!=$previous_id)
                {
                    $pdf->SetFont('Times','B',12);
                    $pdf->Cell(190,5,$data[$i]['patient_name'],0,1,'L');
                    ...
                    for($j=$i;$j<$data_size;$j++)
                    {
                        if($data[$i]['id']==$data[$j]['id'])
                        {
                            $pdf->Row(Array(
                                '',
                                chr(149).' #'.$data[$j]['operation_desc'],
                                date('M d, Y',strtotime($data[$j]['date_performed']))
                            ));
                            // $pdf->Cell(10,5,'',1,0);
                            // $pdf->Cell(155,5,chr(149).' '.$data[$j]['operation_desc'],1,0,'L');
                            // // $pdf->Cell(25,5,$data[$j]['date_performed'],0,1,'C');
                            // $pdf->Cell(25,5,date('M d, Y',strtotime($data[$j]['date_performed'])),1,1,'C');
                        }
                    }
